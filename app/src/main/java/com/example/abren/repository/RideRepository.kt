...
//                    Log.d("Ride Repo: On 200:", response.body().toString())
    ...
    fun createRide(ride: Ride, context: Context): MutableLiveData<Ride?> {
        Log.i("Ride Repo: Create Ride", ride.toString())

        prefs = context.getSharedPreferences("ABREN", Context.MODE_PRIVATE)

        rideService?.createRide(ride, "Bearer ${prefs.getString("TOKEN", null)}")?.enqueue(object :
            Callback<Ride> {
            ...
            override fun onResponse(call: Call<Ride>, response: Response<Ride>) {
                val gson = Gson()
                if (response.code() == 200) {
                    createdData.value = response.body()
                } else {
                    createdData.value = null //TODO: Do for others too
                    Log.d("Ride Repo: Header = ", response.headers().toString())
